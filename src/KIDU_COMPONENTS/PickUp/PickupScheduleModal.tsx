 ...
 *
 * All API calls / field config come from the parent page.
 * Lab      → KiduSelectPopup  (single select)
 * Address  → KiduDropdown     (paginated)
 * Cases    → KiduMultiSelectPopup  (multi-select)
 */

import React, { useState, useEffect } from "react";
import { Modal } from "react-bootstrap";
import "../../Styles/PickUp/PickUpModal.css";
import KiduSelectPopup, { KiduSelectInputPill, type KiduSelectColumn } from "../KiduSelectPopup";
import { useTheme } from "../../ThemeProvider/ThemeProvider";
import KiduDropdown from "../KiduDropdown";
import KiduMultiSelectPopup, { KiduMultiSelectInputPill, type KiduMultiSelectColumn } from "../Kidumultiselectpopup";
...
export interface PickupScheduleModalProps {
  ...
  // Pickup address – KiduDropdown (paginated)
  fetchPickupAddresses: (p: { pageNumber: number; pageSize: number; searchTerm: string }) =>
    Promise<{ data: any[]; total: number }>;
  mapPickupAddress: (row: any) => { value: string | number; label: string };

  // Address details for right panel
...
const INIT: PickupCreateFormData = {
  labMasterId: null, pickUpDate: "",
  pickUpEarliestTime: "", pickUpLateTime: "",
  pickUpAddress: null, caseRegistrationMasterIds: [], trackingNum: "",
};
...
const PickupScheduleModal: React.FC<PickupScheduleModalProps> = ({
  ...
  labSelectEndpoint, labSelectData, labColumns,
  labIdKey = "id", labLabelKey, labSearchKeys,
  fetchPickupAddresses, mapPickupAddress, fetchAddressDetails,
  casesSelectEndpoint, casesSelectData, caseColumns,
  caseIdKey = "id", caseLabelKey, caseSearchKeys,
  onSubmit,
  ...
  useEffect(() => {
    if (show) {
      setForm(INIT); setErrors({}); setLabLabel(""); setCaseLabels({});
      setAddrDetails(null); setSubmitError(null);
    }
  }, [show]);
  ...
  const handleAddressChange = async (val: string | number | null) => {
    set("pickUpAddress", val);
    setAddrDetails(null);
    if (!val) return;
    setLoadingAddr(true);
    try { setAddrDetails(await fetchAddressDetails(val)); }
    catch { /* silent */ }
  ...
  const handleSubmit = async () => {
    ...
    try { await onSubmit(form); onSuccess?.(); onHide(); }
    catch (err: any) { setSubmitError(err?.message ?? "Failed to schedule pickup."); }
      ...
      <Modal show={show} onHide={onHide} size="xl" centered backdrop="static"
        ...
        <Modal.Body className="pickup-modal-body p-0">
          ...
          <div className="pickup-modal-layout">
              ...
              <div className="pickup-form-body">
                ...
                <div className="pickup-field-row">
                  <div className="pickup-field-group">
                    ...
                    />
                    {errors.pickUpDate && <span className="pickup-field-error">{errors.pickUpDate}</span>}
                  </div>
                  ...
                  <div className="pickup-field-group">
                    ...
                    <div className="pickup-time-wrap">
                      ...
                      />
                      {form.pickUpEarliestTime && <span className="pickup-time-badge">{fmt12(form.pickUpEarliestTime)}</span>}
                    </div>
                    {errors.pickUpEarliestTime && <span className="pickup-field-error">{errors.pickUpEarliestTime}</span>}
                  </div>
                  ...
                  <div className="pickup-field-group">
                    ...
                    <div className="pickup-time-wrap">
                      ...
                      />
                      {form.pickUpLateTime && <span className="pickup-time-badge">{fmt12(form.pickUpLateTime)}</span>}
                    </div>
                    {errors.pickUpLateTime && <span className="pickup-field-error">{errors.pickUpLateTime}</span>}
                  </div>
                </div>
                ...
                <div className="pickup-field-group">
                  ...
                  <KiduDropdown
                    ...
                    paginatedFetch={fetchPickupAddresses}
                    mapOption={mapPickupAddress}
                ...
                <div className="pickup-field-group">
                  ...
                  <input type="text" className="pickup-input"
                ...
                <button type="button" className="pickup-submit-btn"
            ...
            <PickupAddressPanel loading={loadingAddr} details={addrDetails} />
          </div>
        </Modal.Body>
      </Modal>

      <KiduSelectPopup
        show={showLabPopup} onClose={() => setShowLabPopup(false)}
        title="Select Lab" subtitle="Choose a lab for this pickup"
        fetchEndpoint={labSelectEndpoint} data={labSelectData}
        columns={labColumns} idKey={labIdKey} labelKey={labLabelKey}
        searchKeys={labSearchKeys} onSelect={handleLabSelect}
      />

      <KiduMultiSelectPopup
        show={showCasePopup} onClose={() => setShowCasePopup(false)}
        title="Select Cases" subtitle="Choose one or more patient cases"
        fetchEndpoint={casesSelectEndpoint} data={casesSelectData}
        columns={caseColumns} idKey={caseIdKey} labelKey={caseLabelKey}
...
export const PickupAddressPanel: React.FC<{
  ...
  <div className="pickup-modal-details-panel">
    ...
    {loading && (
      <div className="pickup-details-loading">
        {[100, 65, 100, 55, 80].map((w, i) => (
          <div key={i}
    ...
    <div className="pickup-details-note">
      ...
      <span><strong>Note:</strong> Requests after 12 PM will be scheduled for the next day</span>
    </div>
  </div>
);

const DR: React.FC<{ icon: string; label: string; value?: string }> = ({ icon, label, value }) => (
