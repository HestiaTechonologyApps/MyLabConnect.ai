...
export interface PickupEditModalProps {
  ...
  // Submit editable fields only
  onSubmit: (id: number | string, data: PickupEditFormData) => Promise<void>;
...
const PickupEditModal: React.FC<PickupEditModalProps> = ({
  show, onHide, onSuccess, recordId,
  fetchRecord, fetchAddressDetails,
  ...
  const [form,          setForm]         = useState<PickupEditFormData>({
    caseRegistrationMasterIds: [], trackingNum: "",
  });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setRecord(null); setAddrDetails(null); setSubmitError(null);
    ...
    (async () => {
      try {
        ...
        // Build initial label map from record
  ...
  const validate = () => {
    ...
    if (!form.trackingNum.trim())               e.trackingNum = "Tracking number is required";
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    try { await onSubmit(recordId, form); onSuccess?.(); onHide(); }
    catch (err: any) { setSubmitError(err?.message ?? "Update failed. Please try again."); }
            ...
            <div className="pickup-modal-form-panel">
              <div className="pickup-modal-header">
                ...
                <div>
                  ...
                  <p className="pickup-modal-subtitle">Update patient cases and tracking information</p>
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="pickup-form-body">
                  ...
                  <div className="pickup-section-divider"><span>Editable Fields</span></div>
                  ...
                  <div className="pickup-field-group">
                    <label className="pickup-field-label">
                      Tracking No. <span className="pickup-required">*</span>
                    </label>
                    <input type="text"
                    ...
                    />
                    {errors.trackingNum && <span className="pickup-field-error">{errors.trackingNum}</span>}
                  </div>
                  ...
                  <button type="button" className="pickup-submit-btn pickup-submit-btn--edit"
...
const ROField: React.FC<{ label: string; value?: string; fullWidth?: boolean }> = ({
  label, value, fullWidth,
}) => (
  <div className={`pickup-readonly-field${fullWidth ? " pickup-readonly-field--full" : ""}`}>
