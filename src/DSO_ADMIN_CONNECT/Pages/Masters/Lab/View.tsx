...
interface Props { show: boolean; onHide: () => void; recordId: string | number; }
...
const fields: ViewField[] = [
  ...
  { name: "authenticationType", label: "Auth Type",           colWidth: 6 },
  { name: "labGroupName",       label: "Lab Group",           colWidth: 6 },
...
const LabMasterViewModal: React.FC<Props> = ({ show, onHide, recordId }) => (
  <KiduViewModal
    ...
    title="View Lab Master" subtitle="Lab Master details"
    fields={fields} recordId={recordId}
    onFetch={(id) => LabMasterService.getById(Number(id))}
    showBadge={true} badgeText="Read Only"
  />
);
